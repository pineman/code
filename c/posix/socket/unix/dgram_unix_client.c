...
int main(void)
{
	...
	// Can use connect(2) and send(2) instead of sendto(2)
	...
	struct timespec t;
	while (1) {
		send_r = sendto(s, (void *) buf, bufsize, 0, (struct sockaddr *) &dest_addr, dest_addrlen);
		...
		recv_r = recv(s, (void *) buf_OK, 50, 0);
		if (recv_r == -1) mperror();
		clock_gettime((clockid_t) CLOCK_REALTIME, &t);
