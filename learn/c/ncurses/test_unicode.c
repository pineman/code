// gcc $(ncursesw6-config --cflags) -Wall -Wextra test.c $(ncursesw6-config --libs
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <wchar.h>
...
wchar_t hor[] = L"│"; // Unicode string

void init_stdscr(WINDOW *stdscr);
...
int main()
{
	...
	setlocale(LC_ALL, ""); // Unset "C" locale
	...
	while (1) {
		...
		switch (c) {
			...
			case '\r': // return key (aka enter)
				...
				move(4, 1);
				clrtoeol();
				mvaddwstr(4, COLS-1, hor); // restore the frame we destroyed
